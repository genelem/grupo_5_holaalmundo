...
Promise.all([servicios, profesores, estudiantes, cantidadIdiomas, cantidadTemas, allGenres, allRol])
           
.then(function([servicios, profesores, estudiantes, cantidadIdiomas, cantidadTemas, allGenres, allRol])
{
let data = {servicios: servicios, 
...
{
return res.render("login/login", {errors: errorsValidation.mapped(),
                                  oldData: req.body})
} 
else 
{
...
req.session.userLogged = usuarioEncontrado                                  // Creamos una Session del usuario

if(req.body.remember_user)
{
res.cookie("userEmail", req.body.email, {maxAge: 60000 * 60 * 24})          // Creamos una cookie para recordar usuario
} 

if(usuarioEncontrado.rol_id == 1)                                           // Redireccionamos una vez pasa la validacion del login
{
...
if(!errorsValidation.isEmpty())
{
return  res.render("register/register", {errors: errorsValidation.mapped(),
                                         oldData: req.body,
                                         allGenres, 
                                         allRol})
} 
